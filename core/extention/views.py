...
class HomeApi(APIView):
    def get(self, request):
        ...
        amazing_offer_product = (
            Product.objects.with_price_info()
            .annotate(ekhtelaf=F("lowest_price_manager") - F("lowest_final_price_manager"))
            .filter(Q(ekhtelaf__gte=1000000) | Q(highest_discount_manager__gte=1))
            .order_by("?")[:20]
        )
        ...
        new_blog_serializer = AllBlogSerializer(new_blog, many=True, context={"request": request})
...
class BlogDetail(APIView):
    def get(self, request, slug):
        ...
        return Response({"blog": blog_serializer.data, "meta_tag": meta_tag_serializer.data}, status=status.HTTP_200_OK)
