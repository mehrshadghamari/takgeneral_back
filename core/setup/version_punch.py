...
if len(sys.argv) >= 2:
    ...
    if "b" in sys.argv:
        ...
        with open(str(os.getcwd()) + "/setup.cfg", "w+") as f:
            content_new = re.sub(
                r"build_number\s=\s\w+", f"build_number = {COMMITID}", content_setup, flags=re.M
            )  # noqa
            f.write(content_new)
        ########## settings.py ##########
        ...
        with open(str(os.getcwd()) + "/version.txt", "w+") as f:
            content_new = re.sub(
                r'BUILD_NUMBER\s=\s"\w+"', f'BUILD_NUMBER = "{COMMITID}"', content_settings, flags=re.M
            )  # noqa
            f.write(content_new)
        #################################
    if "p" in sys.argv:
        subprocess.call(
            ["git", "commit", '-am"version/build number"'], env={"SKIP": "end-of-file-fixer,trailing-whitespace,black"}
        )
